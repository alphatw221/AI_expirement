...
def makeTrainValDataset(dataPath,batchSize,imageSize):
    ...
    trainDataset=tf.data.Dataset.from_tensor_slices((trainFileNames,trainLabels)).shuffle(buffer_size=1000)
    ...
    valDataset=tf.data.Dataset.from_tensor_slices((valFileNames,valLabels)).shuffle(buffer_size=1000)
